...
//---------------------------------------handle
    ...
    private void handleStopButton() {
        ...
        try {
            if (MainActivity.WORK_COUNT_DOWN.isStarted()) {
                MainActivity.WORK_COUNT_DOWN.cancel();
                MainActivity.WORK_DURATION_MP3_PLAYER.close();
            }

            if (MainActivity.RESPITE_COUNT_DOWN.isStarted()) {
                MainActivity.RESPITE_COUNT_DOWN.cancel();
                MainActivity.RESPITE_DURATION_MP3_PLAYER.close();
            }
        } finally {
    ...
    private void handleStartButton() {
        startOrStopLock.lock();
        try {
            MainActivity.WORK_COUNT_DOWN.start();
            MainActivity.WORK_DURATION_MP3_PLAYER.repeatPlayInNewThread();
        } finally {
            startOrStopLock.unlock();
        }
    }
    ...
    private void handleWorkFinished(boolean ableMusic) {
        MainActivity.WORK_DURATION_MP3_PLAYER.close();
        if (ableMusic)
            MainActivity.WORK_FINISHED_MP3_PLAYER.playInNewThread();
        Platform.runLater(() -> {
...
//--------------------------------------- Bind setSetting
    ...
    private void anchorHeightBindAndInit() {
        String heightString = MainActivity.PROPERTIES_MANAGER.getProperty("anchorHeight", Double.toString(anchorPane.getPrefHeight()));
        double height = Double.parseDouble(heightString);
        anchorPane.setPrefHeight(height);
        anchorPane.heightProperty().addListener((observable, oldValue, newValue) -> {
            double anchorHeight = (Double) newValue;
            MainActivity.PROPERTIES_MANAGER.setProperty("anchorHeight", Double.toString(anchorHeight));
        });
    }

    private void anchorWidthBindAndInit() {
        String widthString = MainActivity.PROPERTIES_MANAGER.getProperty("anchorWidth", Double.toString(anchorPane.getPrefWidth()));
        double width = Double.parseDouble(widthString);
        anchorPane.setPrefWidth(width);
        anchorPane.widthProperty().addListener((observable, oldValue, newValue) -> {
            double anchorWidth = (Double) newValue;
            MainActivity.PROPERTIES_MANAGER.setProperty("anchorWidth", Double.toString(anchorWidth));
        });
    }

    private void setRespiteCountDownListener() {
        MainActivity.RESPITE_COUNT_DOWN.startedProperty().addListener((observable, oldValue, newValue) -> {
            Platform.runLater(() -> {
        ...
        MainActivity.RESPITE_COUNT_DOWN.barProgressProperty().addListener((observable, oldValue, newValue) -> {
            Platform.runLater(() -> {
        ...
        MainActivity.RESPITE_COUNT_DOWN.textProgressProperty().addListener((observable, oldValue, newValue) -> {
            Platform.runLater(() -> {
                progressText.setText(newValue);
            });
        });
        MainActivity.RESPITE_COUNT_DOWN.finishedProperty().addListener((observable, oldValue, newValue) -> {
            boolean finished = newValue;
            if (finished) {
                MainActivity.RESPITE_DURATION_MP3_PLAYER.close();
                MainActivity.RESPITE_FINISHED_MP3_PLAYER.playInNewThread();
                Platform.runLater(
                        () -> {
    ...
    private void setWorkCountDownListener() {
        MainActivity.WORK_COUNT_DOWN.startedProperty().addListener((observable, oldValue, newValue) -> {
            Platform.runLater(() -> {
                boolean isStarted = newValue;
                if (isStarted) {
                    startOrStopButton.setText(STOP);
        ...
        MainActivity.WORK_COUNT_DOWN.barProgressProperty().addListener((observable, oldValue, newValue) -> {
            Platform.runLater(() -> {
                progressBar.setProgress((Double) newValue);
        ...
        MainActivity.WORK_COUNT_DOWN.textProgressProperty().addListener((observable, oldValue, newValue) -> {
            Platform.runLater(() -> {
                progressText.setText(newValue);
            });
        });
        MainActivity.WORK_COUNT_DOWN.finishedProperty().addListener((observable, oldValue, newValue) -> {
            boolean finished = newValue;
            if (finished) {
                handleWorkFinished();
            }
...
//--------------------------------------- Method
    ...
    private void developmentMode(Boolean newValue) {
        if (newValue) {
            MainActivity.WORK_COUNT_DOWN.setDuration(MainActivity.DEVELOPMENT_DURATION);
            MainActivity.RESPITE_COUNT_DOWN.setDuration(MainActivity.DEVELOPMENT_DURATION);
        } else {
            MainActivity.WORK_COUNT_DOWN.setDuration(MainActivity.DEFAULT_WORK_DURATION);
            MainActivity.RESPITE_COUNT_DOWN.setDuration(MainActivity.DEFAULT_RESPITE_DURATION);
        }
    }
    ...
    private void addTaskNameAfterFinished(String taskName) {
        ...
        TomatoTask tomatoTask = new TomatoTask(taskName,
                MainActivity.WORK_COUNT_DOWN);
        main.getStackedPanes().addItems(tomatoTask);
        MainActivity.RESPITE_COUNT_DOWN.start();
        MainActivity.RESPITE_DURATION_MP3_PLAYER.repeatPlayInNewThread();
    ...
    public void initHeadText() {
        todayTaskAmount = getCertainDayTaskAmount(MainActivity.getTomatoTasksMap(), LocalDate.now());
        updateHeadText();
    }
